...
echo -e "\e[1;33mWelcome to Arch SteamOS Script, this is a modified test version.\e[0m"
...
paru -S gamescope-session-steam-git --noconfirm --sudoloop
...
echo "[10/18] Installing MangoHUD and Vesktop"
if ! pacman -Qs mangohud > /dev/null; then
    
    sudo pacman -S mangohud vesktop --noconfirm
else
...
echo "adding power button sleep"
if [ -f /etc/systemd/logind.conf.d/logind.conf ]; then
    exit
else
    sudo mkdir /etc/systemd/logind.conf.d
    sudo touch /etc/systemd/logind.conf.d/logind.conf
    echo "[Login]" | sudo tee -a /etc/systemd/logind.conf.d/logind.conf
